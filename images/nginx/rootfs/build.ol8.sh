...
yum install -y bash \
  ...
  which \
  libcurl-devel \
  git pkgconf flex bison yajl lmdb-devel libtool autoconf libxml2 pcre-devel libxml2-devel \
  python38 \
  libmaxminddb-devel \
  bc \
  unzip \
  dos2unix mercurial \
  https://yum.oracle.com/repo/OracleLinux/OL7/latest/x86_64/getPackage/GeoIP-1.5.0-13.el7.x86_64.rpm \
